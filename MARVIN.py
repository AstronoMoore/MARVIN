...
def plot_marvin(marvin_results):
    ...
    discovery_mjd = Time(marvin_results.info['discoverydate'].item(), format='iso', scale='utc').mjd
    ...
    for i in order:
        if i in bands_for_offset:
            ...
            if offset < 0: 
                ...
                offset_str = '+'
            patch = ax.add_patch(patches.Rectangle((0, 0), 0, 0, facecolor=c, label=i+' '+offset_str+str(offsets[i])))
            count+=1
    ...
    # bands 
    ax.legend(handles = handle_list, title="Bands", ncol=8, prop={'size': 12}, fontsize=10, title_fontsize=10, markerscale=1.7,loc='upper center', bbox_to_anchor=(0.5, -0.12),fancybox=True, shadow=False,)
    ...
    for telescope in lightcurve['telescope'].unique():
        ...
        handle = ax.errorbar([],[],y_err = None,color = 'grey',marker = markersyle, linestyle = '', label = telescope,fillstyle='none')
        handle_list.append(handle)

    fig.legend(handles=handle_list,title="Telescopes", ncol=8, prop={'size': 12}, fontsize=10, title_fontsize=12, markerscale=1.7,loc='upper center', bbox_to_anchor=(0.5, -0.01),fancybox=True, shadow=False)
    plt.tight_layout()
...
def fetch_ztf(ztf_name):
    ...
    data_ouput = data.filter(['mjd','fid','magpsf','sigmapsf','diffmaglim'])
...
def fetch_gaia(gaia_name):
    print('Fetching GAIA')
    gaia_alerts = 'http://gsaweb.ast.cam.ac.uk/alerts/alert/' +str(gaia_name)+ '/lightcurve.csv/'
    ...
    data_output.insert(len(data_output.columns),'e_magnitude',value='0.1') # properly calculate GAIA mags!!! 
    data_output.insert(len(data_output.columns),'telescope',value='Gaia')
    data_output.insert(len(data_output.columns),'magnitude',value=data_output['averagemag'].astype(float).round(3))
    data_output = data_output.filter(['time','band','magnitude','e_magnitude','telescope'])
    return data_output
...
def clean_atlas(name):
    ...
    try: 
        ATLAS_orange = pd.read_csv(data_dir +name+ ".o.1.00days.lc.txt", delim_whitespace=True)
    ...
    try:
        ATLAS_cyan = pd.read_csv(data_dir +name+ ".c.1.00days.lc.txt", delim_whitespace=True)
...
def clean_panstarrs(panstarss):
    ...
    for group_name, df_group in grouped:
        ...
        panstarss_output.loc[count] = [working_frame['#mjd'].astype(float).mean(), mag, mag_err, filter]
        count +=1

    panstarss_output.insert(len(panstarss_output.columns),'telescope','Pan-STARRS')
...
class TNS_interogate:
    def __init__(self, TNS_name):
        ...
        response = requests.post(url, data=data, headers={'User-Agent':'tns_marker{"tns_id":165250,"type": "bot", "name":"MARVIN"}'})
        ...
        if response.status_code == 200:
            ...
            tns_object_info = pd.DataFrame([object_TNS_data.values()], columns=object_TNS_data)
            ...
            self.internal_names = tns_object_info['internal_names'].apply(lambda x: x.split(','))[:]
...
class MARVIN:
     ...
     def __init__(self,TNS_NAME):
        ...
        if TNS_object.ztf is not None: 
            ...
            try: 
                fetch_ztf_cone(TNS_object.info['radeg'].item(),TNS_object.info['decdeg'].item())
                self.ztf_cone_data = fetch_ztf_cone(TNS_object.info['radeg'],TNS_object.info['decdeg'])
                self.data = pd.concat((self.data,self.ztf_cone_data))
            except LasairError as e:
                print(e)

        if TNS_object.pan_starrs is not None:
            self.pan_starrs_data = fetch_panstarrs_forced(TNS_object.pan_starrs)
        ...
        self.neowise_data = fetch_neowise(TNS_object.info['radeg'].item(),TNS_object.info['decdeg'].item())
...
if __name__ == "__main__": 
    Lasair_token = "336663f982474a379a934539e0d09860f6cb69cb"
    tns_api_key = 'cb0fa7b0ddcc1ed5a4f18a82c1e01fb9092985ec'
    tns_api_headers = str({'User-Agent':'tns_marker{"tns_id":165250,"type": "bot", "name":"MARVIN"}'})
    create_config(Lasair_token,tns_api_key,tns_api_headers)
    marvin_results = MARVIN(sys.argv[1])
    print(marvin_results.data)
    marvin_results.data.to_csv(sys.argv[1] + '.csv', index=False)
